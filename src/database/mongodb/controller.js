    ...
    */
    ...
    if (!!join){
        ...
        return Promise.all(prom).then(_results=>{
            ...
            return _validateTbl(tbl).create(data).then(r=>{
                return _get(tbl, {_id:r._id});
            });
        });
    } else {
        return _validateTbl(tbl).create(data).then(r=>{
...
function _get(tbl, param){
    // console.log(param,110)
    let {_id} = _validateParam('get', param);
    return _validateTbl(tbl).findOne({_id}).select('-__v').lean().then(r=>{
        return _clean(r);
    });
};

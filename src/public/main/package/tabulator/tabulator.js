...
;(function (global, factory) {
	...
	Tabulator.prototype.updateTableOptions = function(options){
	...
	Tabulator.prototype._setData = function (data, params, config, inPosition, columnsChanged) {
		...
		// console.log(this.options.ajaxRequestFunc.toString());
	...
	Tabulator.prototype.extendModule = function (name, property, values) {
		...
		if (Tabulator.prototype.moduleBindings[name]) {
			...
			if (source) {
				...
				} else if (source instanceof Function && values instanceof Function){
					//to make the function extendible
					Tabulator.prototype.moduleBindings[name].prototype[property] = values;
	...
	var Ajax = function Ajax(table) {
		...
		this.msgElement = this.createMsgElement(this.table.options.ajaxConfig.loaderMsg); //message element
	...
	Ajax.prototype.createMsgElement = function (template) {
		...
		if (template){
			if (typeof template == 'string'){
				...
				let el = p.children[0];
				if (!el){throw new Error('loadMsg in AjaxConfig is not an HTML')}
				return el;
			} else if (template.toString().includes('Element')){
				return temlpate;
			}
		} else {
	...
	Ajax.prototype.loadData = function (inPosition, columnsChanged) {
		var self = this;
		// console.log(this.progressiveLoad);
	...
	Ajax.prototype.sendRequest = function (silent) {
		...
		// console.log(_this43.loaderPromise.toString());
	...
	Ajax.prototype.showLoader = function () {
		...
		if (shouldLoad) {
			...
			if (!this.table.options.ajaxConfig.loaderMsg){
			...
			// console.log(this.loaderElement, this.msgElement)
